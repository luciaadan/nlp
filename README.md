...
```
---
## The project was done in WSL/MacOS and it requires a virtual environment to be run. The necessary files are in the repository but it also needs to be activated on the device so the following command must be executed in the terminal:
```bash
uv sync
